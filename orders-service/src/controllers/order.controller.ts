...
const BOOKS_API = process.env.BOOKS_API || "http://localhost:3000";
...
export class OrderController {
  ...
  async create(
    ...
    const response = await fetch(`${BOOKS_API}/books/${order.bookId}`);
    if (!response.ok) {
      throw new HttpErrors.UnprocessableEntity(
        `Book with id ${order.bookId} does not exist`,
      );
    }
